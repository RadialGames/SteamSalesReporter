 ...
 * Or import it and call the functions directly
 * 
 * Example:
 *   await checkApp(1149000)
 */

import { sql } from '../lib/db/sqlite.js';

export async function checkApp(appId) {
  console.log(`\n=== Checking App ID ${appId} ===\n`);

  // Query by app_id only
  const appIdResult = (await sql`
    SELECT 
  ...
  const allPackageActivations = (await sql`
    SELECT 
      COALESCE(SUM(ABS(gross_units_activated)), 0) as gross_activated,
      COUNT(*) as total_records,
  ...
  const zeroAppIdActivations = (await sql`
    SELECT 
      COALESCE(SUM(ABS(gross_units_activated)), 0) as gross_activated,
      COUNT(*) as total_records
    FROM parsed_sales
  ...
  const difference = (allPackageActivations?.gross_activated ?? 0) - (appIdResult?.gross_activated ?? 0);
  console.log(`\n=== Summary ===`);
