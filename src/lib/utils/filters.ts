...
export function applyFilters(records: SalesRecord[], filters: Filters): SalesRecord[] {
  ...
  // Single app filter (legacy - for backwards compatibility)
 ...
 * 
 * @example
 * // Get app summary without filtering by the selected app
 * const records = applyFiltersExcluding(sales, filters, 'appId', 'appIds');
 */
