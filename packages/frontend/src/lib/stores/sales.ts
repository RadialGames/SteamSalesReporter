...
function getUnits(s: { unitsSold?: number; netUnitsSold?: number }): number {
  return s.unitsSold ?? s.netUnitsSold ?? 0;
}
...
export const dailySummary = derived([salesStore, debouncedFilters], ([$sales, $filters]) => {
  ...
  for (const sale of filtered) {
    ...
    existing.totalUnits += getUnits(sale);
...
export const appSummary = derived([salesStore, debouncedFilters], ([$sales, $filters]) => {
  ...
  for (const sale of filtered) {
    const appId = sale.appId;
    if (appId == null) continue;
    const existing = byApp.get(appId) || {
      appId,
      appName: sale.appName || `App ${appId}`,
      totalRevenue: 0,
      totalUnits: 0,
    };
...
export const countrySummary = derived([salesStore, debouncedFilters], ([$sales, $filters]) => {
  ...
  for (const sale of filtered) {
    const countryCode = sale.countryCode;
...
export const totalStats = derived([salesStore, debouncedFilters], ([$sales, $filters]) => {
  ...
  return {
    ...
    uniqueApps: new Set(filtered.map((s) => s.appId).filter((id): id is number => id != null)).size,
    uniqueCountries: new Set(filtered.map((s) => s.countryCode).filter((c): c is string => c != null)).size,
  };
});
