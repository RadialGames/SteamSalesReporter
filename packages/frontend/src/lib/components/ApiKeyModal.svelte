<script lang="ts">
  ...
  import type { ApiKeyInfo, DataProgressCallback } from '$lib/services/types';
  ...
  async function handleWipeAllData() {
    ...
    try {
      const onProgress: DataProgressCallback = (message, progress) => {
        wipeProgressMessage = message;
        wipeProgressPercent = progress;
      };
      await services.clearAllData(onProgress);
  ...
  async function handleWipeProcessedData() {
    ...
    try {
      ...
      await services.clearProcessedData(onProgress);
  ...
  async function handleDeleteKey(keyId: string) {
    ...
    try {
      ...
      await services.clearDataForKey(keyId, onProgress);
        ...
        {#if confirmAction?.type === 'wipeProcessed'}
          <div class="bg-amber-500/20 border border-amber-500/50 rounded-lg p-4 mb-4">
            ...
            <p class="text-xs text-amber-300 mb-4">
              ...
              will be kept and can be reprocessed by running the CLI tool's fetch command.
            </p>
            <div class="flex gap-2">
              <button
                type="button"
